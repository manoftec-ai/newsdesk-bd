...
## Work in Progress
- [x] **GH Actions font outage + fix (2026-09-20)**: new runner image lost `fonts-noto-bengali` in apt → `author.yml` + `images.yml` died at the font step all day → **0 posts published**. Root cause: `E: Unable to locate package fonts-noto-bengali` (exit 100). Fix D35: bundled static TTFs in `pipeline/fonts/` + `fontconfig.local.conf`, workflows export `FONTCONFIG_FILE` (no apt). **Second bug fixed in same push**: commit steps used `git diff --quiet` + `git pull --rebase` BEFORE `git add` → "cannot pull with rebase: You have unstaged changes" (exit 128) on the images rerun even though 34 webp rendered (0 failed); now `git status --porcelain` → add → commit → pull --rebase → push in both author.yml+images.yml. **Still blocked for PUBLISHING**: repo has NO `LLM_API_KEY` (or `OPENCODE_API_KEY` for auto-author.yml) secret → scheduled authoring is a no-op by design. 13 briefs queued (international-134, national-122/123/125/126/127/128/130/131/136, sports-129/133). See memory/sessions/2026-09-20-gh-actions-font-outage.md
- [IN PROGRESS — 2026-09-20] **Living Story Tracker (D36), increments 1–2 DONE**: inc-1 Story Registry `site/src/data/tracked-stories.json` (meta.schemaVersion 1 + stories[] with slug/title/category/trackedAt/status/lastChecked/nextCheck/fingerprint/reason) + helper lib `site/src/lib/tracked-stories.js` + news schema +`tracked`(bool default false)+`lastChecked`(date). inc-2 article UI: `.tracked-chip` (→/tracked) + "শেষ চেক" stamp + `.tracker-box` "এই গল্পটি ট্র্যাক করা হচ্ছে" block; `.tracked-chip` #4f46e5 + `.tracker-box` in styles.css. Seeded dengue + national-119 (updated NOT set yet — honest). Next: increment 3 `/tracked` hub page + nav/footer links; watcher (daily Google News RSS search) = later increment.
