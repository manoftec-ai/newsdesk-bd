...
- [IN PROGRESS — 2026-09-20] **Living Story Tracker (D36), increment 1 DONE**: Story Registry `site/src/data/tracked-stories.json` (meta.schemaVersion 1 + stories[] with slug/title/category/trackedAt/status/lastChecked/nextCheck/fingerprint/reason), helper lib `site/src/lib/tracked-stories.js` (trackedStories/trackedStoriesBySlug/isTracked/getTrackedStory/trackedRegistryMeta), news schema +`tracked` (bool default false) +`lastChecked` (date optional); seeded dengue + national-119 as tracked. Next: increment 2 article UI (tracked badge + শেষ চেক + আপডেট ইতিহাস), increment 3 `/tracked` hub + nav. Watcher (daily Google News RSS search) = later increment.
