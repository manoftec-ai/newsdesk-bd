...
## Images v2 — REAL newspaper photos (same session, user decision 2026-09-20)
- USER: "in BD we don't have strict DMCA/image credit problem... put real image from google/newspapers... without real photo our news will not be trustworthy."
- Agreed + built: new priority in `lib/images.mjs#chooseThumbnail`: (1) **source og:image** — fetch the story's FIRST source article HTML, extract `og:image`/`twitter:image`, require https + same-outlet/CDN registrable domain, download WITH Referer → brand overlay → WebP; credit `ছবি: <source name>` (BD standard, no license string). (2) Openverse precise fallback (unchanged, credited). (3) branded card last.
- `add_images.mjs` now parses `sources:` front matter list and passes it through; fixed a parser bug (`/m` `$` matched before every newline so the block regex stopped after the first line).
- Result: national-125 → ছবি: চ্যানেল আই, national-128 → ছবি: চ্যানেল আই, national-127 → ছবি: প্রথম আলো, international-134 → ছবি: চ্যানেল আই, sports-129 → ছবি: প্রথম আলো (all from private og:image of the SAME verified articles). dry-run confirmed mode="source" for all 5; npm test 10/10.
- Re-branded via dispatched images.yml run (db6535a). ALL 5 images + articles live (200) after a manual `vercel --prod` from site/ (b24xg4t0b) — NOTE: the parallel session's new deploy.yml workflow ran "success" but its 15:45/46 deploys did NOT make images live on prod; manual CLI deploy did. → **verify deploy.yml actually ships image/bot commits** next time (possible bug).
- Standing image philosophy now: real outlet photos first, never hotlink Google Images (their URLs are just newspaper CDNs), never rip watermarks, auto-credit every photo.
