// test/facebook.test.mjs — unit tests for facebook_post helpers
import { test } from 'node:test';
import assert from 'node:assert/strict';
import { mkdtempSync, writeFileSync, rmSync } from 'node:fs';
import { tmpdir } from 'node:os';
import { join } from 'node:path';
import { readFrontmatter, latestUnsent } from '../tools/facebook_post.mjs';
...
test('readFrontmatter parses yaml front matter', () => {
  const dir = mkdtempSync(join(tmpdir(), 'facebook-test-'));
  try {
